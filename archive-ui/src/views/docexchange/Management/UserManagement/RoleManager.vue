<template>
  <div>
    <el-dialog :title="$t('application.selectUser')" :visible.sync="selectUserDialogVisible" width="820px">
      <UserSelector
        ref="UserSelector"
    ...
    <el-row>
      <el-col :span="12">
        ...
        >
          ...
          <el-table-column :label="$t('application.userName')" min-width="30%" sortable>
            <template slot-scope="scope">
              <span>{{scope.row.name}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.explain')" min-width="30%">
            <template slot-scope="scope">
      ...
      <el-col :span="12">
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane :label="$t('route.userManager')" name="usertab">
            <el-row>
              <el-col :span="8">
                <el-input
              ...
              <el-col :span="16" style="text-align:left;">
                ...
                >{{$t('application.addusers')}}</el-button>
              </el-col>
            </el-row>
            <el-table :data="userList" border :height="tableHeight2" style="width: 100%">
              ...
              <el-table-column :label="$t('application.userName')" width="150" sortable>
                <template slot-scope="scope">
                  <span>{{scope.row.name}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('application.loginUserName')" min-width="20%" sortable>
                <template slot-scope="scope">
                  <span>{{scope.row.loginName}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('application.compName')" min-width="20%">
                <template slot-scope="scope">
                  <span>{{scope.row.companyName}}</span>
                </template>
              </el-table-column>
               <el-table-column :label="$t('application.group')" min-width="20%">
                <template slot-scope="scope">
                  <span>{{scope.row.groupName}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('application.operation')" width="90">
                <template slot-scope="scope">
                  <el-button
                    :plain="true"
                    type="danger"
                  ...
                  >{{$t('application.removeusers')}}</el-button>
                </template>
              </el-table-column>
            </el-table>
            <el-pagination
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then(function(response) {
          if(response.data.code==9){
             let msg = _self.$t('message.hasnoPermssion')
            _self.$message({ showClose: true, message: msg, duration: 2000, type: "warning"})
            return
          }
          if(response.data.code==0){
             let msg = _self.$t('message.operationFaild')
            _self.$message({ showClose: true, message: msg, duration: 2000, type: "warning"})
            return
          }
          if(response.data.code==1001){
             let msg = _self.$t('message.timeOut')
            _self.$message({ showClose: true, message: msg, duration: 2000, type: "warning"})
            return
          }
          let msg = _self.$t('message.RemoveUserSuccess')
    ...
    refreshData() {
      ...
      var m = new Map();
      m.set("groupType", 2);
      m.set("id", this.currentUser().userId);
      m.set("condition", _self.inputkey);
      m.set("pageSize", _self.pageSize);
      m.set("pageIndex", _self.currentPage - 1);
      // console.log('pagesize:', _self.pageSize);
      axios
        .post("/exchange/getGroups", JSON.stringify(m))///admin/getGroups
        .then(function(response) {
    ...
    refreshUserData() {
      ...
      m.set("condition"," COMPANY_NAME='"+this.currentUser().company+"' ");
