                    ...
                    <template slot="paneR">
                        ...
                        <template v-if="isProject">
                            <el-row>
                                ...
                                <el-form :inline="true" :model="filters" @submit.native.prevent>
                                    <el-form-item>
                                        <el-input width="100px" v-model="filtersPLAN.title" :placeholder="$t('application.wbs')+$t('application.or')+$t('application.Title')" @keyup.enter.native='searchItemPlan'></el-input>
                                    </el-form-item>
                                    <el-form-item>
                                ...
                                <split-pane v-on:resize="onSplitResize" :min-percent='20' :default-percent='topPercent' split="horizontal">
                                    <template slot="paneL">
                                        <DataGrid
                                            ...
                                            condition=" TYPE_NAME='计划任务' and SUB_TYPE='Activity'"
                                            :isshowCustom="true"
                                            :isInitData="false"
                                            :isEditProperty="false"
                        ...
                        <template v-if="isICM">
                            ...
                            <div :style="{position:'relative',height: layout.height-startHeight+'px'}">
                                <split-pane v-on:resize="onSplitResize" :min-percent='20' :default-percent='topPercent' split="horizontal">
                                    ...
                                    <template slot="paneR">
                                <el-tabs  value="t01">
                                    <el-tab-pane :label="$t('application.InterfaceTransfer')" name="t01">
                                        <el-button type="primary" v-on:click="exportDataSubTable($refs.ICMTransfer)">{{$t('application.ExportExcel')}}</el-button>
                                            ...
                                            <DataGrid
                                                ...
                                                <el-tab-pane :label="$t('application.InterfaceOpinion')" name="t02">
                                                    <!-- 导出Excel -->
        ...
        nodeClick(data,node,current){
            ...
            if(node.parent.data.name=="文函"){//文函查询
                ...
                if(node.data.name=='ICM'){
                    ...
                    _self.$nextTick(()=>{
                        if(user.userType==2 && user.company!=null){
                            _self.$refs.PlanDataGrid.condition=_self.condition=" TYPE_NAME='计划任务' and SUB_TYPE='Activity' "
                            +" and C_PROJECT_NAME = '"+_self.projectName+"' AND (C_COMPANY='"+user.company +"'"
                            +" or C_TO like'%"+user.company+"%')";
