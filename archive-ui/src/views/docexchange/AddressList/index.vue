    ...
    <el-dialog :title="dialogtitle" :visible.sync="dialogVisible" width="70%">
      <el-form :model="form" label-position="right" label-width="100px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item
              :label="$t('application.loginUserName')"
              :label-width="formLabelWidth"
              :rules="[{ required: true, message: '必填', trigger: 'blur'}]"
            >
            {{form.loginName}}
          ...
          <el-col :span="12">
            <el-form-item :label="$t('application.email')" :label-width="formLabelWidth">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('application.phone')" :label-width="formLabelWidth">
                <template v-if="showTextBox()">
                    <el-input
                        v-model="form.phone"
                        auto-complete="off"
          ...
          <el-col :span="12">
            <el-form-item :label="$t('application.whetherOn')" style="float:left;" :label-width="formLabelWidth">
                <div>
                    {{form.isActived}}
                </div>
              <!-- <el-select
          ...
          <el-col :span="12">
            <el-form-item :label="$t('application.compName')" :label-width="formLabelWidth">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('application.proMain')" :label-width="formLabelWidth">
              <div>{{form.projectName}}</div>
             
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item :label="$t('application.explain')" :label-width="formLabelWidth">
                <div>
                    {{form.description}}
                </div>
              <!-- <el-input
      ...
      >
        ...
        <el-button type="primary" v-if="showTextBox()" @click="additem(form)">{{$t('application.ok')}}}</el-button>
      </div>
    </el-dialog>
    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="dataList" border :height="tableHeight" style="width: 100%">
          ...
          <el-table-column :label="$t('application.userName')" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.name}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.loginUserName')" width="120">
            <template slot-scope="scope">
              <span>{{scope.row.loginName}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.email')" min-width="15%">
            <template slot-scope="scope">
              <span>{{scope.row.email}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.phone')" min-width="10%">
            <template slot-scope="scope">
              <span>{{scope.row.phone}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.explain')" min-width="15%">
            <template slot-scope="scope">
              <span>{{scope.row.description}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.proMain')" min-width="15%">
            <template slot-scope="scope">
          ...
          -->
          <el-table-column :label="$t('application.whetherOn')" width="90">
            <template slot-scope="scope">
              <span>{{scope.row.isActived?"是":"否"}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('application.operation')" width="70">
            <template slot-scope="scope">
               <el-button
                    type="primary"
                    plain
