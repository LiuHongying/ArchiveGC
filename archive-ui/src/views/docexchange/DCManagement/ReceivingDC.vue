<template>
    <DataLayout>
        ...
        <template v-slot:main="{layout}" :style="{position:'relative',height:layout.height-startHeight+'px'}">
                        ...
                        <el-tabs v-model="selectedTabName">
                            <el-tab-pane :label="$t('application.TransferDoc')" name="t01" v-if="isShowDesgin">
                                ...
                                <DataGrid
                                    ...
                                    v-bind:tableHeight="(layout.height-startHeight)*(100-topPercent)/100-bottomHeight+'px'"
                                    v-bind:isshowOption="true" v-bind:isshowSelection ="true"
...
export default {
    ...
    data(){
        return{
            // 本地存储高度名称
            topStorageName: 'ReceivingDCHeight',
            // 非split pan 控制区域高度
            startHeight: 135,
            // 顶部百分比*100
            topPercent: 65,
    ...
    methods: {
      ...
      window.open(href.href, "_blank");
        ...
        showItemStyle(val){
            // (scope.data['C_PROCESS_STATUS']!=null&&scope.data['C_PROCESS_STATUS']=='已解锁')?
            //             {'background':'red'}:''
            if(val.row['C_PROCESS_STATUS']!=null&&val.row['C_PROCESS_STATUS']=='已解锁'){
