<template>
    <DataLayout>
        <template v-slot:header>
            <!-- 待提交文函 -->
            <!-- 设计文件附件 -->
            <el-dialog :title="$t('application.Attachment')" :close-on-click-modal="false" :visible.sync="dialog.visible" width="50%" :before-close="handleClose">      
                <AttachmentFile ref="subAttachment" :docId="docId"></AttachmentFile>
            </el-dialog>
            <!-- 批量导入 -->
            <el-dialog :title="$t('message.Batch')+' '+$t('application.Import')+$t('application.document')" :close-on-click-modal="false" :visible.sync="batchDialogVisible" width="80%" >
            ...
            <!-- 创建附件 -->
            <el-dialog :title="$t('application.Import')" :visible.sync="importdialogVisible" :close-on-click-modal="false" width="70%">
            ...
            <!-- 相关文件创建选择IED-->
            <el-dialog :title="dialog.title" :visible.sync="propertyrela" width="80%" :close-on-click-modal="false" :before-close="handleClose"> 
                <DataLayout>
                    <template v-slot:header>
                        <el-form>
                        <el-form-item>
                            ...
                            <el-tab-pane :label="$t('application.TransferDoc')" name="t01" v-if="isShowDesgin">
                                <el-row>
                                    ...
                                    <el-form :inline="true" :model="filters" @submit.native.prevent>
                                        ...
                                        <el-form-item>
                                        <el-button type="primary" @click="beforImport($refs.transferDoc,true,'设计文件')">{{$t('application.Import')}}</el-button>
                                        </el-form-item>
                                        <el-form-item>
                                        <el-button type="primary" @click="applyForUnlock">{{$t('application.applyForUnlock')}}</el-button>
                                        </el-form-item>
                                        <el-form-item v-if="isShowMountFile" >
                                            <MountFile :selectedItem="selectedTransferDocItems" @refresh='refreshReleventDocData'>{{$t('application.ReplaceDoc')}}</MountFile>
                                        </el-form-item>
                                        <el-form-item v-if="isShowMountFile">
                                ...
                                <DataGrid
                                    ...
                                    >
                                        <template slot="sequee" slot-scope="scope">
                                            ...
                                            &&scope.data.row['C_PROCESS_STATUS']=='已解锁')?{'background':'red'}:''">{{(scope.currentPage-1) * scope.pageSize+ scope.data.$index+1}}</span>
                                        </template>
                                ...
                                <el-row>
                                    <el-col :span="24">
                                        <el-form :inline="true" :model="filters" @submit.native.prevent>
                                            ...
                                            <el-form-item>
                                                <MountFile :selectedItem="relevantDocSelected" @refresh='refreshTransferDocData'>{{$t('application.ReplaceDoc')}}</MountFile>
                                            </el-form-item>
                                            <el-form-item>
...
export default {
    ...
    data(){
        return{
            // 本地存储高度名称
            topStorageName: 'RejectedDCHeight',
            // 非split pan 控制区域高度
            startHeight: 135,
            // 顶部百分比*100
            topPercent: 65,
...
                        mp.set("C_REF_CODING",'C_REF_CODING');
                        mp.set("C_ITEM_DATE",'C_ITEM_DATE');
                        
                        dt["C_REF_CODING"] = _self.selectRow["CODING"];
                        dt["C_ITEM_DATE"] = _self.selectRow["C_ITEM_DATE"];

                        _self.$refs.ShowProperty.setMainSubRelation(mp);
                        _self.$refs.ShowProperty.setMainObject(dt);
                        _self.$refs.ShowProperty.loadFormInfo();
                        return 
                           }
                    })}
        ...
        selectChangeTransferDoc(val) {
            let _self=this;
            _self.selectedTransferDocItems = val;
            val.forEach(element => {
                if(element['C_PROCESS_STATUS']=='已解锁'){
        ...
        applyForUnlock(){
                ...
                if(_self.selectedTransferDocItems.length==0){
                    _self.$message({
                                showClose: true,
                                message: "请选择一条或多条要申请解锁的数据！",
                                duration: 2000,
                                type: 'warning'
                                });
                            return;
                ...
                let i;
                let m = [];
                let tab=_self.selectedTransferDocItems;
                for (i in tab) {
                    if(tab[i]["C_PROCESS_STATUS"]!=null&&tab[i]["C_PROCESS_STATUS"]=='已解锁'){
                        continue;
                    }
                    m.push(tab[i]["ID"]);
                }
                if(m.length>0){
                    axios.post("/dc/applyForUnlock",JSON.stringify(m),{
                            headers: {
                                "Content-Type": "application/json;charset=UTF-8"
                            }
                        })
