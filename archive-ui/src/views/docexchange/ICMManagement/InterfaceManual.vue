<template>
  <DataLayout>
    ...
    <el-dialog
      :title="$t('application.Import') + ' ICM'"
    ...
    <el-dialog :title="延误打开反馈" :visible.sync="Visible1" width="80%">
      ...
      ></ShowProperty>
      <div slot="footer" class="dialog-footer">
        <el-button @click="SaveOpenFeedBack">{{
          $t("application.ok")
        }}</el-button>
    ...
    <el-dialog :title="延误关闭反馈" :visible.sync="Visible2" width="80%">
      ...
      ></ShowProperty>
      <div slot="footer" class="dialog-footer">
        <el-button @click="SaveCloseFeedBack">{{
          $t("application.ok")
        }}</el-button>
        <el-button @click="Visible2 = false">{{
          $t("application.cancel")
        }}</el-button>
      </div>
    </el-dialog>
      ...
      <el-form :inline="true" :model="forms.headForm">
        ...
        <el-form-item>
          <el-input
            ...
            :placeholder="
              $t('message.pleaseInput') + $t('application.codingDesc')
            "
          ></el-input>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-button
              type="default"
              @click.native="exportData('ICM', 'ICMGrid')"
              >{{ $t("application.ExportExcel") }}</el-button
            >
        </el-form-item>
        <el-form-item v-if="roles1">
          
          <el-button type="primary" @click="newArchiveItem('ICM')">{{
            $t("application.new")
          }}</el-button>
          <el-button
            type="primary"
            @click="beforImport($refs.mainDataGrid, '/系统配置/导入模板/ICM')"
        ...
        <el-form-item v-if="roles2">
          <el-button
            type="primary"
            @click="icmOpenfeedback('延误打开反馈', selectedItems)"
            >{{ $t("application.delayOpF") }}</el-button
          >
        </el-form-item>
        <el-form-item v-if="roles2">
          <el-button
            type="primary"
            @click="icmClosefeedback('延误关闭反馈', selectedItems)"
            >{{ $t("application.delayClF") }}</el-button
          >
        </el-form-item>
      </el-form>
    </template>
        ...
        >
          <template slot="paneL">
            ...
            >
              <template slot="customMoreOption" slot-scope="scope">
                <el-button
                  type="primary"
                  @click.native="showReplyDialog(scope.data.row)"
                  size="mini"
                  >{{ $t("application.transmit") }}</el-button
                >
              </template>
            </DataGrid>
          </template>
          <template slot="paneR">
            <el-tabs v-model="tabs.active">
              <el-tab-pane
                :label="$t('application.InterfaceTransfer')"
                name="ICMPass"
              >
                <el-button
                  type="default"
                  @click.native="exportData('ICMPass', 'ICMPassGrid')"
                  >{{ $t("application.ExportExcel") }}</el-button
                >
                <DataGrid
                  ref="ICMPass"
                  showOptions="查看内容"
                  v-bind="tables.ICMPass"
                  :tableHeight="
              ...
              <el-tab-pane
                :label="$t('application.InterfaceOpinion')"
                name="ICMComments"
              >
                <el-form>
                  <el-form-item>
                    <el-button
                      ...
                      align="center"
                      @click.native="
                        exportData('ICMComments', 'ICMCommentsGrid')
                      "
                      >{{ $t("application.ExportExcel") }}</el-button
                    >
                  </el-form-item>
  ...
  methods: {
    showReplyDialog(row) {
      ...
      this.$nextTick(() => {
        if (_self.$refs.replyProperty) {
          axios
            .post("/exchange/doc/getReplyInfo", _self.replyDocId)
    ...
    saveReplyItem() {
      ...
      _self.validateData(m, function (isOk) {
        ...
        axios
          .post("/exchange/doc/newReplyDoc", formdata, {
    ...
    loadsuccess() {
      if (this.currentUser().company != this.ownerCompany) {
        this.tables.main.condition +=
          " AND (C_CODE5='" +
          this.currentUser().companyCode1 +
          "' OR C_CODE6='" +
          this.currentUser().companyCode1 +
          "')";
        this.userCondition =
    ...
    onDataGridRowClick: function (row) {
      var condition1 =
        "SELECT CHILD_ID from ecm_relation where TYPE_NAME='接口信息传递单'and PARENT_ID ='" +
        row.ID +
        "'";
      ...
      var condition2 =
        "SELECT CHILD_ID from ecm_relation where TYPE_NAME='接口信息意见单'and PARENT_ID ='" +
        row.ID +
        "'";
    ...
    search() {
      ...
      if (_self.selectiteam && _self.selectiteam.length > 0) {
        _self.$refs.mainDataGrid.condition =
          _self.userCondition +
          " TYPE_NAME='ICM' and C_PROJECT_NAME in(" +
          _self.selectiteam +
          ") AND (CODING LIKE '%" +
          _self.inputValueNum +
          "%' OR C_COMMENT LIKE '%" +
        ...
        _self.$refs.mainDataGrid.condition =
          _self.userCondition +
          " TYPE_NAME='ICM' and C_PROJECT_NAME = '@project' AND (CODING LIKE '%" +
          _self.inputValueNum +
          "%' OR C_COMMENT LIKE '%" +
          _self.inputValueNum +
          "%')";
      ...
      if(_self.advCondition!= undefined && _self.advCondition.length>0){
        _self.$refs.mainDataGrid.condition +=" and " + _self.advCondition
      }
    ...
    saveItem() {
      ...
      if (_self.$refs.ShowProperty.myItemId == "") {
        axios
          .post("/exchange/ICM/newICM", formdata, {
            "Content-Type": "multipart/form-data",
          })
          .then(function (response) {
            let code = response.data.code;
    ...
    SaveOpenFeedBack() {
      ...
      if (_self.$refs.ShowProperty.myItemId == "") {
        axios
          .post("/exchange/ICM/OpenFeedBack", formdata, {
            "Content-Type": "multipart/form-data",
          })
          .then(function (response) {
            let code = response.data.code;
    ...
    SaveCloseFeedBack() {
      ...
      if (_self.$refs.ShowProperty.myItemId == "") {
        axios
          .post("/exchange/ICM/CloseFeedBack", formdata, {
            "Content-Type": "multipart/form-data",
          })
          .then(function (response) {
            let code = response.data.code;
    ...
    onPropertiesSaveSuccess(data) {
      ...
      formdata.append("metaData", JSON.stringify(data));
      axios
        .post("/exchange/ICM/UpdataICMLogger", formdata)
        .catch(function (error) {
          _self.$message(_self.$t("message.failured"));
          console.log(error);
        });
