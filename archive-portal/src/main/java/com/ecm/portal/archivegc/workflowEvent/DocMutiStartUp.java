...
@Component(value = "MutiDocListener")
public class DocMutiStartUp implements ExecutionListener, JavaDelegate, TaskListener {
	...
	private Map<String, Object> loadProcessBusinessLogicToVariable(IEcmSession ecmSession,
		...
		try {
			...
			varMap.put("assigneeListA",
					getApprover(ecmSession, ecmObject.getAttributes().get("C_REVIEWER1").toString()));	//鉴定工作小组
			varMap.put("assigneeListB",
					getApprover(ecmSession, ecmObject.getAttributes().get("C_REVIEWER2").toString()));	//鉴定领导小组
